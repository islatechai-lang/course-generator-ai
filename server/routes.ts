...
async function deleteUploadThingMedia(fileKey: string, userId: string, fileSize: number) {
  try {
    await utapi.deleteFiles(fileKey);
    console.log(`Deleted fileKey ${fileKey} from UploadThing`);
  } catch (error) {
    console.error(`Failed to delete fileKey ${fileKey} from UploadThing:`, error);
  }

  // Deduct from user storage
  const user = await storage.getUser(userId);
  if (user) {
    const newUsedStorage = Math.max(0, (user.usedStorage || 0) - fileSize);
    await storage.updateUser(userId, { usedStorage: newUsedStorage });
    console.log(`Deducted storage for user ${userId}: new usedStorage = ${newUsedStorage}`);
  }
}
...
async function authenticateWhop(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  ...
  try {
    ...
    req.isPro = isPro;
    req.isBasic = isBasic;
    console.log(`[Backend] authenticateWhop: user ${result.userId}, isPro=${isPro}, isBasic=${isBasic}`);

    let user = await storage.getUserByWhopId(result.userId);
    const calculatedPlan = isPro ? "pro" : (isBasic ? "basic" : "free");
    ...
    if (!user) {
      ...
      if (user.plan !== calculatedPlan) {
        updates.plan = calculatedPlan;
        needsUpdate = true;
      }

      const whopUserData = await getWhopUser(result.userId);
      if (!user.profilePicUrl && whopUserData?.profile_picture?.url) {
        updates.profilePicUrl = whopUserData.profile_picture.url;
        needsUpdate = true;
      }
...
export async function registerRoutes(
  ...
  app.get("/api/dashboard/:companyId", authenticateWhop, requireAdmin, async (req: AuthenticatedRequest, res: any) => {
    try {
      ...
      const generationLimit = await getGenerationLimit(req.user.id, req.isPro, req.user.whopUserId, req.isBasic);
  ...
  app.post("/api/dashboard/:companyId/courses/generate", authenticateWhop, requireAdmin, async (req: AuthenticatedRequest, res: any) => {
    try {
      ...
      if (remaining <= 0) {
        if (!req.isPro && !req.isBasic) {
          return res.status(403).json({
            error: "Free users can't use AI generation. Please upgrade!",
            needsUpgrade: true
          });
        }
    ...
    try {
      ...
      if (published === true && course.published === false) {
        ...
        if (publishedCount >= maxPublished) {
          ...
          return res.status(403).json({
            error: `Publishing limit reached. ${planName} plan allows up to ${maxPublished} published courses.`,
  ...
  app.delete("/api/dashboard/:companyId/courses/:courseId", authenticateWhop, requireAdmin, async (req: AuthenticatedRequest, res: any) => {
    try {
      ...
      // Retrieve course details with lessons to clean up videos
      const courseWithModules = await storage.getCourseWithModules(course.id);
        ...
        for (const mod of courseWithModules.modules) {
          if (mod.lessons) {
            for (const les of mod.lessons) {
              if (les.media) {
                for (const m of les.media) {
                  if (m.type === "video" && m.fileKey) {
                    deleteUploadThingMedia(m.fileKey, req.user.id, m.fileSize || 0).catch(console.error);
                  }
                }
              }
            }
  ...
  app.delete("/api/dashboard/:companyId/lessons/:lessonId/media/:mediaId", authenticateWhop, requireAdmin, async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const mediaItemToDelete = (lesson.media || []).find(m => m.id === req.params.mediaId);
      if (mediaItemToDelete && mediaItemToDelete.type === "video" && mediaItemToDelete.fileKey) {
        deleteUploadThingMedia(mediaItemToDelete.fileKey, req.user.id, mediaItemToDelete.fileSize || 0).catch(console.error);
      }
    ...
    try {
      ...
      if (published === true && !course.published) {
        ...
        if (publishedCount >= limit) {
          ...
          return res.status(403).json({
            error: `Publishing limit reached. ${planName} users can publish up to ${limit} course${limit > 1 ? "s" : ""}.`,
            needsUpgrade: !req.isPro
          });
        }
      }
  ...
  // Basic Plan Checkout
  app.post("/api/basic/checkout", authenticateWhop, async (req: AuthenticatedRequest, res: any) => {
    try {
      if (req.isPro) {
        return res.status(400).json({ error: "You are already a Pro user" });
      }
      if (req.isBasic) {
        return res.status(400).json({ error: "You are already a Basic user" });
      }

      const checkoutResult = await createProCheckoutSession(BASIC_PLAN_ID);
      if (!checkoutResult) {
